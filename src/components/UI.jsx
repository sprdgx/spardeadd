...
const audios = {
  background: new Audio("/audios/SPRD-Wave (Groovepad).wav"),
  punch: new Audio("/audios/shoot.mp3"),
  shield: new Audio("/audios/shield.mp3"),
  grab: new Audio("/audios/collect.mp3"),
...
export const UI = () => {
    ...
    case "playerChoice":
      label =
        currentPlayer.id === me.id
          ? "Select the player you want to shoot"
          : `${currentPlayer?.state.profile?.name} is going to shoot on someone`;
      break;
    case "playerAction":
      switch (currentCard) {
        case "shoot":
          label = actionSuccess
            ? `${currentPlayer?.state.profile?.name} is shooting ${target?.state.profile?.name}`
            : `${currentPlayer?.state.profile?.name} failed shooting ${target?.state.profile?.name}`;
          break;
        case "grab":
          label = actionSuccess
          ...
          break;
        case "shield":
          label = `${currentPlayer?.state.profile?.name} can't be shut until next turn`;
          break;
        default:
          break;
      }
  ...
  useEffect(() => {
    if (audioEnabled) {
      audios.background.volume = 0.2; // Set volume to 50%
